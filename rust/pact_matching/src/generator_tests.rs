...
fn applies_the_generator_to_a_json_map_entry() {
  ...
  json_handler.apply_key(RefCell::new(query_result.clone()), &s!("$.b"), &Generator::RandomInt(0, 10));
  p!(query_result);
...
fn json_generator_handles_invalid_path_expressions() {
  ...
  json_handler.apply_key(RefCell::new(query_result.clone()), &s!("$["), &Generator::RandomInt(0, 10));
...
fn does_not_apply_the_generator_when_field_is_not_in_map() {
  ...
  let json_handler = JsonHandler { value: map.clone() };
  let mut query_result = QueryResult::default(map.clone());
  json_handler.apply_key(RefCell::new(query_result.clone()), &s!("$.d"), &Generator::RandomInt(0, 10));
