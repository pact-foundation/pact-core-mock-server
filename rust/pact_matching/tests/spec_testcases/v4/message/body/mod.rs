...
fn unexpected_index_with_not_null_value() {
    ...
    let expected = Message::from_json(0, &pact.get("expected").unwrap(), &PactSpecification::V4).unwrap();
    println!("EXPECTED: {:?}", expected);
    let actual = Message::from_json(0, &pact.get("actual").unwrap(), &PactSpecification::V4).unwrap();
