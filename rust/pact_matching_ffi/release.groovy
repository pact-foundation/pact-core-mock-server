...
def prevTag = 'git describe --abbrev=0  --tags --match=libpact_matching_ffi-*'.execute().text.trim()
def changelog = []
...
ask('Tag and Push commits?: [Y]') {
  ...
  executeOnShell("git tag libpact_matching_ffi-v${releaseVer}")
...
ask("Bump version to $nextVer?: [Y]") {
  ...
  executeOnShell "sed -i -e 's/documentation = \"https:\\/\\/docs\\.rs\\/pact_matching_ffi\\/${releaseVer}\\/pact_matching_ffi\\/\"/documentation = \"https:\\/\\/docs\\.rs\\/pact_matching_ffi\\/${nextVer}\\/pact_matching_ffi\\/\"/' Cargo.toml"
